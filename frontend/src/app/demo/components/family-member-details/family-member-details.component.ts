...
export class FamilyMemberDetailsComponent implements OnInit {
  ...
  authSetting(property: any,id_property: any) {
    ...
    this._confirmationService.confirm({
      ...
      accept: () => {
        this._messageService.add({ severity: 'info', summary: 'Confirmed', detail: 'Authorization updated!', life: 3000 });
        let authObject = {
        ...
        this._userService.updateFamilyAuth(this.token, authObject).subscribe({
          next: res => {
      ...
      reject: () => {
        this._messageService.add({ severity: 'error', summary: 'Rejected', detail: 'Action cancelled', life: 3000 });
      }
    });
